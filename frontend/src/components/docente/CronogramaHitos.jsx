...
function ModalCrearHito({ miembros, onGuardar, onCancelar }) {
    ...
    function validar() {
        const e = {}
        if (!form.nombre.trim())
            e.nombre = 'El nombre del hito es obligatorio.'
        ...
        if (!form.fecha_fin)
            e.fecha_fin = 'La fecha de fin es obligatoria.'
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-[2px] flex items-center justify-center z-50 p-4"
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden"
                ...
                <div className="flex items-start justify-between p-6 border-b border-[#e1e3e4]">
                    <div>
                        <h2 className="text-[18px] font-bold text-[#191c1d]">Nuevo hito</h2>
                        <p className="text-[13px] text-[#9ba7ae] mt-0.5">Define un hito para el cronograma del equipo</p>
                    </div>
                    <button onClick={onCancelar}
                        className="w-8 h-8 rounded-xl text-[#9ba7ae] hover:bg-[#f0f2f3] flex items-center justify-center transition-colors">
                        <IconX />
                    </button>
                </div>
                ...
                <div className="p-6 flex flex-col gap-4 max-h-[60vh] overflow-y-auto">
                    ...
                    <div className="flex flex-col gap-1.5">
                        <label className="text-[13px] font-semibold text-[#191c1d] pl-1">
                            Nombre del hito <span className="text-[#d32f2f]">*</span>
                        </label>
                        <input
                            type="text"
                            value={form.nombre}
                            onChange={e => set('nombre', e.target.value)}
                            placeholder="ej: Entrega del diseño de base de datos"
                            className={errores.nombre ? err : ok}
                        />
                    ...
                    {/* Descripción */}
                    <div className="flex flex-col gap-1.5">
                        <label className="text-[13px] font-semibold text-[#191c1d] pl-1">Descripción <span className="text-[#9ba7ae] font-normal">(opcional)</span></label>
                        <textarea
                            value={form.descripcion}
                            onChange={e => set('descripcion', e.target.value)}
                            placeholder="Describe el alcance o criterios de aceptación..."
                            rows={3}
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div className="flex flex-col gap-1.5">
                            <label className="text-[13px] font-semibold text-[#191c1d] pl-1">
                                Fecha inicio <span className="text-[#d32f2f]">*</span>
                            </label>
                            <input
                                type="date"
                                ...
                                onChange={e => {
                                    ...
                                    // Si fecha_fin ya está puesta y ahora es inválida, limpiar su error
                                    if (form.fecha_fin && e.target.value >= form.fecha_fin)
                                        setErrores(er => ({ ...er, fecha_fin: 'La fecha de fin debe ser posterior a la de inicio.' }))
                                    else
                                        setErrores(er => ({ ...er, fecha_fin: '' }))
                                }}
                                className={errores.fecha_inicio ? err : ok}
                            />
                            {errores.fecha_inicio && <p className="text-[12px] text-[#ba1a1a] pl-1">{errores.fecha_inicio}</p>}
                        </div>
                        <div className="flex flex-col gap-1.5">
                            <label className="text-[13px] font-semibold text-[#191c1d] pl-1">
                                Fecha fin <span className="text-[#d32f2f]">*</span>
                            </label>
                            <input
                                type="date"
                                ...
                                className={errores.fecha_fin ? err : ok}
                            />
                            {errores.fecha_fin && <p className="text-[12px] text-[#ba1a1a] pl-1">{errores.fecha_fin}</p>}
                        </div>
                    </div>

                    {/* Aviso de validación cruzada visible */}
                    {form.fecha_inicio && form.fecha_fin && form.fecha_fin <= form.fecha_inicio && !errores.fecha_fin && (
                        <div className="flex items-center gap-2 px-3 py-2 bg-[#fff8f7] border border-[#ffdad6] rounded-xl text-[12px] text-[#ba1a1a]">
                    ...
                    {/* Responsable */}
                    <div className="flex flex-col gap-1.5">
                        <label className="text-[13px] font-semibold text-[#191c1d] pl-1">Responsable <span className="text-[#9ba7ae] font-normal">(opcional)</span></label>
                        <select
                            ...
                            {miembros.map(m => (
                                <option key={m.id} value={m.usuario_id}>{m.nombre_completo}</option>
                            ))}
                        </select>
                    </div>
                    ...
                    {/* Estado */}
                    <div className="flex flex-col gap-1.5">
                        <label className="text-[13px] font-semibold text-[#191c1d] pl-1">Estado inicial</label>
                        <div className="flex flex-wrap gap-2">
                            {ESTADOS.map(est => (
                                <button key={est.key}
                                    type="button"
                                    onClick={() => set('estado', est.key)}
                                    className={`flex items-center gap-1.5 px-3 py-2 rounded-xl border-2 text-[13px] font-semibold transition-all ${
                                        form.estado === est.key
                ...
                <div className="flex gap-2 p-4 bg-[#f8f9fa] border-t border-[#e1e3e4]">
                    ...
                    <button onClick={handleGuardar}
                        className="flex-1 h-11 rounded-xl bg-[#d32f2f] text-white font-semibold text-[14px] hover:bg-[#af101a] transition-colors flex items-center justify-center gap-2">
                        <IconPlus /> Crear hito
                    </button>
                </div>
            </div>
...
function FilaHito({ hito, meses, onEliminar }) {
    ...
    return (
        <div className="flex border-b border-[#f0f2f3] last:border-0 hover:bg-[#fafbfc] transition-colors group">
            {/* Columna info */}
            <div className="w-52 flex-shrink-0 px-4 py-3 border-r border-[#f0f2f3]">
                <div className="flex items-start justify-between gap-1">
                    <p className="text-[13px] font-semibold text-[#191c1d] truncate leading-tight">{hito.nombre}</p>
                    <button onClick={() => onEliminar(hito.id)}
                        className="opacity-0 group-hover:opacity-100 w-6 h-6 rounded-lg text-[#9ba7ae] hover:text-[#ba1a1a] hover:bg-[#ffdad6] flex items-center justify-center transition-all flex-shrink-0">
                        <IconTrash />
                    </button>
                </div>
                <p className="text-[11px] text-[#9ba7ae] mt-0.5 leading-tight">
                    {formatFecha(hito.fecha_inicio)}
                    <span className="mx-1">—</span>
                    {formatFecha(hito.fecha_fin)}
                </p>
                <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-semibold mt-1 ${info.bg} ${info.text}`}>
            ...
            <div className="flex-1 flex min-w-0">
                {meses.map((m, i) => {
                    const mesStart = new Date(m.año, m.mes, 1)
                    const mesEnd   = new Date(m.año, m.mes + 1, 0)

                    const overlap = start <= mesEnd && end >= mesStart
                    if (!overlap) {
                        return <div key={i} className="flex-1 border-r border-[#f0f2f3] last:border-0" />
                    }

                    const diasMes  = mesEnd.getDate()
                    const diaStart = start > mesStart ? start.getDate() - 1 : 0
                    const diaEnd   = end   < mesEnd   ? end.getDate()       : diasMes
                    const leftPct  = (diaStart / diasMes) * 100
                    const widthPct = ((diaEnd - diaStart) / diasMes) * 100
                        ...
                        <div key={i} className="flex-1 border-r border-[#f0f2f3] last:border-0 relative py-4 px-1">
                            <div
                                ...
                                style={{
                                    left:            `${leftPct}%`,
                                    width:           `${widthPct}%`,
                                    backgroundColor: info.bar,
                                    opacity:         0.85,
                                }}
                                className="absolute top-1/2 -translate-y-1/2 h-5 rounded-md min-w-[4px]"
                            />
                        </div>
                    )
                })}
...
// ── Fila placeholder (cuando no hay hitos) ────────────────────────────────────
...
function FilaPlaceholder({ meses, altPattern }) {
    return (
        <div className="flex border-b border-[#f0f2f3] last:border-0 opacity-30 pointer-events-none">
            <div className="w-52 flex-shrink-0 px-4 py-3 border-r border-[#f0f2f3]">
                <div className="h-3 w-28 bg-[#e1e3e4] rounded-full" />
                <div className="h-2 w-20 bg-[#f0f2f3] rounded-full mt-2" />
            </div>
            <div className="flex-1 flex">
                {meses.map((_, i) => (
                    <div key={i} className="flex-1 border-r border-[#f0f2f3] last:border-0 relative py-4 px-1">
                        {((altPattern ? i === 2 : i === 1) || (altPattern ? i === 4 : i === 3)) && (
                            <div className="absolute top-1/2 -translate-y-1/2 h-5 rounded-md bg-[#e1e3e4]"
                                style={{ left: '5%', width: altPattern ? '80%' : '90%' }} />
                        )}
                    </div>
                ))}
            </div>
...
export default function CronogramaHitos() {
    ...
    // Hitos persistidos por proyecto en localStorage (sin backend)
    const storageKey = `hitos_proyecto_${proyectoId}`
    ...
    useEffect(() => {
        localStorage.setItem(storageKey, JSON.stringify(hitos))
    }, [hitos, storageKey])

    useEffect(() => { cargarInfo() }, [proyectoId])
    ...
    async function cargarInfo() {
        ...
        try {
            ...
            setInfo({ proyecto: data.proyecto, curso: data.curso, equipo: data.equipos?.[0] ?? null })
        } catch {
    ...
    const meses  = generarMeses(inicio, 6)
    ...
    if (error) return (
        <div className="flex-1 flex flex-col items-center justify-center gap-3 text-center px-4">
            ...
            <button onClick={cargarInfo} className="h-9 px-4 rounded-xl bg-[#d32f2f] text-white text-[13px] font-semibold hover:bg-[#af101a] transition-colors">
                Reintentar
            </button>
        </div>
    )
    ...
    return (
        <div className="flex-1 overflow-y-auto p-6" style={{ fontFamily: "'Manrope', sans-serif" }}>
            ...
            <div className="mb-4 flex items-center gap-2 text-[13px] flex-wrap">
                ...
                <span className="ml-1 px-2 py-0.5 rounded-full text-[11px] font-bold bg-[#e8f5e9] text-[#2e7d32]">HU-014</span>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-[#e1e3e4] p-5 mb-5">
                <div className="flex items-center gap-4 flex-wrap">
                    <div className="w-12 h-12 rounded-xl flex items-center justify-center text-white text-[18px] font-bold flex-shrink-0"
                        ...
                        {(equipo?.nombre ?? proyecto?.nombre ?? 'E')[0].toUpperCase()}
                    </div>
                    <div className="flex-1 min-w-0">
                        <h1 className="text-[20px] font-bold text-[#191c1d] leading-tight">
                            {equipo?.nombre ?? 'Sin equipo'}
                        </h1>
                        <p className="text-[13px] text-[#9ba7ae] mt-0.5">
                            {proyecto?.nombre ?? '—'}{curso?.nombre ? ` · ${curso.nombre}` : ''}
                        </p>
                    </div>
                    {miembros.length > 0 && (
                        <div className="flex -space-x-2 flex-shrink-0">
                            ...
                            {miembros.length > 5 && (
                                <div className="w-8 h-8 rounded-full border-2 border-white bg-[#f0f2f3] flex items-center justify-center text-[10px] font-semibold text-[#4c616c]">
                                    +{miembros.length - 5}
                                </div>
                            )}
                        </div>
                    ...
                    <button
                        ...
                        className="h-10 px-4 rounded-xl bg-[#d32f2f] text-white font-semibold text-[13px] hover:bg-[#af101a] transition-colors flex items-center gap-2 shadow-sm flex-shrink-0">
                        <IconPlus /> Nuevo hito
                    </button>
                </div>

                {/* Stats */}
                <div className="flex items-center gap-5 mt-4 pt-4 border-t border-[#f0f2f3] flex-wrap">
                    <div className="flex items-center gap-1.5 text-[13px] text-[#5b403d]">
                        <svg className="w-4 h-4 text-[#9ba7ae]" viewBox="0 0 16 16" fill="currentColor">
                            <path d="M8 8a3 3 0 100-6 3 3 0 000 6zm-6 6s-1 0-1-1 1-4 7-4 7 3 7 4-1 1-1 1H2z"/>
                        </svg>
                        <strong>{miembros.length}</strong> miembro{miembros.length !== 1 ? 's' : ''}
                    </div>
                    <div className="flex items-center gap-1.5 text-[13px] text-[#5b403d]">
                        ...
                        <strong>{hitos.length}</strong> hito{hitos.length !== 1 ? 's' : ''} definido{hitos.length !== 1 ? 's' : ''}
                    </div>
                    <div className="flex items-center gap-1.5 text-[13px] text-[#5b403d]">
                        <svg className="w-4 h-4 text-[#388e3c]" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round">
                            <circle cx="8" cy="8" r="6"/><path d="M5 8l2 2 4-4"/>
                        </svg>
                        <strong>{completados}</strong> completado{completados !== 1 ? 's' : ''}
                    </div>
                </div>
            </div>
            ...
            <div className="flex items-center gap-2 flex-wrap mb-4">
                {ESTADOS.map(e => (
                    <span key={e.key}
                        className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[12px] font-semibold ${e.bg} ${e.text}`}>
            ...
            <div className="bg-white rounded-2xl border border-[#e1e3e4] overflow-hidden">
                ...
                <div className="flex items-center justify-between px-5 py-3 border-b border-[#e1e3e4] bg-[#f8f9fa]">
                    <h2 className="text-[14px] font-bold text-[#191c1d]">
                        {meses[0].short} {meses[0].año} — {meses[meses.length - 1].short} {meses[meses.length - 1].año}
                    </h2>
                    <div className="flex items-center gap-1">
                        <button onClick={() => setOffsetMeses(o => o - 1)}
                            className="w-8 h-8 rounded-lg text-[#4c616c] hover:bg-[#e1e3e4] flex items-center justify-center transition-colors">
                            <IconLeft />
                        </button>
                        <button onClick={() => setOffsetMeses(0)}
                            className="h-7 px-3 rounded-lg text-[12px] font-semibold text-[#4c616c] hover:bg-[#e1e3e4] transition-colors">
                            Hoy
                        </button>
                        <button onClick={() => setOffsetMeses(o => o + 1)}
                            className="w-8 h-8 rounded-lg text-[#4c616c] hover:bg-[#e1e3e4] flex items-center justify-center transition-colors">
                ...
                <div className="flex border-b border-[#e1e3e4]">
                    <div className="w-52 flex-shrink-0 px-4 py-2.5 border-r border-[#e1e3e4]">
                        <p className="text-[11px] font-bold text-[#9ba7ae] uppercase tracking-wide">Hito</p>
                    </div>
                    {meses.map((m, i) => {
                        const esHoy = m.mes === hoy.getMonth() && m.año === hoy.getFullYear()
                        return (
                            <div key={i}
                                className={`flex-1 py-2.5 text-center border-r border-[#e1e3e4] last:border-0 ${esHoy ? 'bg-[#fff8f7]' : ''}`}>
                ...
                {/* Filas de hitos o empty state */}
                {hitos.length > 0 ? (
                    hitos.map(hito => (
                        <FilaHito key={hito.id} hito={hito} meses={meses} onEliminar={eliminarHito} />
                    ))
                ) : (
                    <>
                        ...
                        <div className="flex flex-col items-center justify-center py-10 px-6 text-center">
                            <div className="w-14 h-14 rounded-2xl bg-[#f0f2f3] flex items-center justify-center mb-3">
                                <svg className="w-7 h-7 text-[#9ba7ae]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                                    <rect x="3" y="4" width="18" height="18" rx="2"/><path d="M3 9h18M9 4v5M15 4v5M8 14h4M8 17h6"/>
                                </svg>
                            </div>
                            <h3 className="text-[15px] font-bold text-[#191c1d] mb-1">No hay hitos definidos</h3>
                            <p className="text-[13px] text-[#9ba7ae] max-w-xs leading-relaxed mb-4">
                                Crea el primer hito para visualizarlo en el cronograma del equipo.
                            </p>
                            <button
                                onClick={() => setModalAbierto(true)}
                                className="inline-flex items-center gap-2 h-9 px-4 rounded-xl bg-[#d32f2f] text-white font-semibold text-[13px] hover:bg-[#af101a] transition-colors">
                                <IconPlus /> Crear primer hito
                            </button>
                        </div>
                        <FilaPlaceholder meses={meses} altPattern={false} />
                    </>
                    ...
                    <div className="flex flex-wrap gap-3">
                        {miembros.map((m, i) => (
                            <div key={m.id} className="flex items-center gap-2.5 px-3 py-2 bg-[#f8f9fa] rounded-xl border border-[#e1e3e4]">
                                <div style={{ backgroundColor: AVATAR_COLORS[i % AVATAR_COLORS.length] }}
                                    className="w-7 h-7 rounded-full flex items-center justify-center text-white text-[10px] font-bold flex-shrink-0">
                                    {m.iniciales}
                                </div>
                                <div>
                                    <p className="text-[12px] font-semibold text-[#191c1d] leading-tight">{m.nombre_completo}</p>
                                    {m.rol_interno && <p className="text-[11px] text-[#9ba7ae] capitalize">{m.rol_interno}</p>}
                                </div>
                            </div>
                        ))}
                    </div>
